document.addEventListener('DOMContentLoaded', () => {
  ...
  genBtn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('/generate', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({notes: txt})});
  ...
  saveBtn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('/save', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({qa: lastGenerated})});
      const data = await res.json();
      if (data.saved) {
